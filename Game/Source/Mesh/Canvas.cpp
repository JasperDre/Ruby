...
void Canvas::AddVert(vec2 objectPos, float objectAngle, vec2 objectScale, vec2 aUVscale, vec2 aUVoffset)
{
	for (unsigned int i = 0; i < m_BaseTileVerts.size(); i++)
	{
		if (IsReversed == true)
		{
			...
			VertexFormat vertex = VertexFormat(vec2(finalPos), MyColor(m_BaseTileVertsReverse.at(i).m_Color), vec2(UVTemp));

			m_CanvasVerts->push_back(vertex);
		}
		else
		{
			...
			VertexFormat vertex = VertexFormat(vec2(finalPos), MyColor(m_BaseTileVerts.at(i).m_Color), vec2(UVTemp));
